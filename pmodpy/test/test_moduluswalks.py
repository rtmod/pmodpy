...
def test_walks_shakeri_1a():
    ...
    assert abs(moduluswalks.modulus_walks(Shakeri_1a, 0, 8, eps=2e-36, verbose=0,p=2)[0]-0.4)<1E-4;


def test_walks_shakeri_1b():
    Shakeri_1b=examplegraphs.Shakeri_1b();
    assert abs(moduluswalks.modulus_walks(Shakeri_1b, 0, 8, eps=2e-36, verbose=0,p=2)[0]-0.5)<1E-4;


def test_walks_shakeri_1d():
    Shakeri_1d=examplegraphs.Shakeri_1d();
    print(Shakeri_1d)
    assert abs(moduluswalks.modulus_walks( Shakeri_1d, 0, 8, eps=2e-36,p=2)[0]-0.5169)<1E-4
