...
def minimum_walk_usage(graph, source, target, dens=None):
    """
    Given a graph, return a *numpy* array of dimension |E(G)| indicating
    the usage of each edge by a minimum walk from source to target.

    Uses `get_shortest_paths` from *python-igraph*:
    http://igraph.org/python/
    (Minimum walks are necessarily shortest paths.)
...
def modulus_walks_density(graph, source, target, p=2,
    ...
    """
    Compute the walk modulus of a graph.
    ...
    # Store a minimum walk under the given edge weights
    z = minimum_walk_usage(graph, source, target)
    ...
    while continue_criterion(z, dens):
        ...
        # Calculate a minimum walk under the new extremal density estimate
        z = minimum_walk_usage(graph, source, target, dens=dens)
...
def modulus_walks(graph, source, target, p=2,
                  subfamily=False,
                  eps=1e-8, solver=cvxpy.CVXOPT, verbose=False):
    #
